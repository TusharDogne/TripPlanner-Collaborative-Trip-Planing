...
const HeroSection = () => {
  ...
  /**
   * Debounces the search by waiting 300ms before calling fetchPlaces.
   */
  useEffect(() => {
    if (search.trim()) {
      const timer = setTimeout(() => fetchPlaces(search), 300);
      return () => clearTimeout(timer);
    } else {
      setSuggestions([]); // Clear suggestions instantly if search is empty
    }
  }, [search]);
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-background via-card to-muted overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        ...
        <div
          className="absolute top-40 right-20 w-24 h-24 bg-secondary rounded-full blur-2xl animate-pulse"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className="absolute bottom-40 left-1/4 w-40 h-40 bg-accent rounded-full blur-3xl animate-pulse"
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[80vh]">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              >
                Transform trip planning from a stressful solo task into an engaging
                group experience. Plan together, budget smart, and discover more
            ...
            <motion.div
              ...
              className="bg-card border border-border rounded-2xl p-6 shadow-soft relative"
            >
              ...
              <div className="space-y-4">
                <div className="flex space-x-3 relative">
                  <div className="flex-1">
                    ...
                    {suggestions.length > 0 && (
                      <div className="absolute z-50 top-14 left-0 w-full bg-card border border-border rounded-lg shadow-lg max-h-60 overflow-y-auto">
                        {suggestions.map((place) => (
                          <div
                            key={place.id}
                            onClick={() => handleSelect(place)}
                            className="px-4 py-2 hover:bg-accent/10 cursor-pointer text-sm"
                          >
                ...
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>Popular: Indore, Rajasthan, Agra, Manali</span>
                  <span className="text-success font-medium">
                    {tripsToday} trips started today
                  </span>
                </div>
              </div>
            </motion.div>
          ...
          >
            ...
            <div className="relative bg-card border border-border rounded-2xl p-6 shadow-collaborative">
              ...
              <div className="space-y-4">
                {[
                  { name: "Madhya Pradesh", votes: 3, total: 3 },
                  { name: "Rajasthan", votes: 2, total: 3 },
                  { name: "Taj-Mahal", votes: 3, total: 3 },
                ].map((destination, i) => (
                  <motion.div
              ...
              <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    <span className="font-medium text-primary">Vaidehi:</span>
                    <span className="text-muted-foreground ml-2">
                      Love the temple idea! 🏯
                    </span>
                  </div>
                  <div>
                    <span className="font-medium text-secondary">Yash:</span>
                    <span className="text-muted-foreground ml-2">
                      Budget looks good so far 💰
                    </span>
                  </div>
                </div>
              </div>
